...
@RestController
@RequestMapping(path = "/v1/usuarios/{idUsuario}/grupos", produces = MediaType.APPLICATION_JSON_VALUE)
public class UsuarioGrupoController implements UsuarioGrupoControllerOpenApi {
    ...
    public CollectionModel<GrupoModel> listar(@PathVariable Long idUsuario) {
        ...
        if (algaSecurity.podeEditarUsuariosGruposPermissoes()) {
            gruposModel.add(algaLinks.linkToUsuarioGrupoAssociacao(idUsuario, "associar"));

            gruposModel.getContent().forEach(grupoModel -> {
                grupoModel.add(algaLinks.linkToUsuarioGrupoDesassociacao(idUsuario, grupoModel.getId(), "desassociar"));
            });
        }
